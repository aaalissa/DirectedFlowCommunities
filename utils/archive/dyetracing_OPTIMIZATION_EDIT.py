...
class Node:
    def __init__(self, name, dye_concentration=0.0, source_node=False, absorption_rate=0.5, infinite_source=False, absorption_limit = np.inf):
        self.name = name                    #best if it aligns with index in node list, but can be named anything
...
class Edge:
    def __init__(self, start_node, end_node, length = 1, velocity = 1, decay_rate = 0, min_dt = 0.01):
...
def simulate(network_nodes, network_edges, T, print_final=False, history=True, step_mode='equal'):
    """
...
def normalize(absorbed_history, source_node=0):
    """
    ...
    global_max = np.max(np.delete(absorbed_history, source_node, axis=1))
       
    if global_max == 0:
        print("Warning: global maximum concentration is 0, cannot normalize. Check decay rate !")
        return absorbed_history

    for i in range(absorbed_history.shape[0]):
        # set the value of the source node to 1
        absorbed_history[i][source_node] = 1
...
def run_simulation(node_list, edge_list, source_node_index, timesteps, infinite_source, init_dye_concentration, step_mode = 'proportional', troubleshooting_params = None):
    """
    run simulation for a single node specifying source node, return the normalized absorbed history
    inputs:
        node_list (list): list of Node objects
        edge_list (list): list of Edge objects
        source_node (int): index of source node in node_list
        timesteps (int): number of timesteps to run simulation
        infinite_source (bool): whether source node is infinite source
        init_dye_concentration (float): initial dye concentration of source node
        step_mode (str): 'equal' or 'proportional'
        troubleshooting_params (dict): dictionary of parameters for troubleshooting
    output:
        norm_absorbed_history (list): normalized absorbed history
    """
    reset_simulation(node_list, edge_list)
    set_source_node(node_list, source_node_index, {"infinite_source": infinite_source, "initial_dye_concentration": init_dye_concentration})

    #check node attributes for troubleshooting
    if troubleshooting_params is not None:
        check_node_attributes(node_list, troubleshooting_params, source_node=source_node_index)
        
    absorbed_history, _ = simulate(node_list, edge_list, T = timesteps, print_final=False, history=True, step_mode = step_mode)
    norm_absorbed_history = normalize(absorbed_history, source_node=source_node_index)

    return norm_absorbed_history[-1].tolist()
...
def set_source_node(node_list, source_node_index, params):
    ...
    assert "infinite_source" in params or "initial_dye_concentration" in params, "Must specify either infinite_source or initial_dye_concentration"
    ...
    if "infinite_source" in params:
        node_list[source_node_index].infinite_source = params["infinite_source"]
    if "initial_dye_concentration" in params:
        node_list[source_node_index].dye_concentration = params["initial_dye_concentration"]
...
def find_steady_state(concentration_history, threshold=1e-5):
    """
    Determine the time step at which the system reaches a steady state.

    Parameters:
    - concentration_history: A list of lists. Each inner list represents the dye concentrations of all nodes at a particular time step.
    - threshold: The maximum allowable change in concentration to consider the system to have reached a steady state.

    Returns:
    - The time step at which the system reaches a steady state, or None if the system does not reach a steady state within the provided concentration history.
    """
    
    for t in range(3, len(concentration_history)):
        relative_changes = [abs(a - b) / (abs(b) + 1e-10) for a, b in zip(concentration_history[t], concentration_history[t-1])]
